 ...
 *
 */
public class UpdateUser extends APIExcutor {

	/**
	 * 执行参数
	 */
	private UpdateUserParamsModel _params;

	/**
	 * 执行参数设定
	 */
	public UpdateUser setParams(UpdateUserParamsModel params) {
		// 参数设定
		this._params = params;
	...
	public APIResultModel run() {
		...
		// API的URL
		String url = CnfPropsUtil.getProperty("wx.io.qyapi.url.user.updateuser");

		// 如果是测试的情况
		if (!StringUtils.isEmpty(_params.test) && "true".equals(_params.test)) {
			url = CnfPropsUtil.getProperty("wx.io.qyapi.test.url.user.updateuser");
		}
