 ...
 */
package com.inbody.crm.common.security.shiro.session;
...
public class CacheSessionDAO extends EnterpriseCacheSessionDAO implements SessionDAO {
    ...
    /**
	 * 获取活动会话
	 * @param includeLeave 是否包括离线（最后访问时间大于30分钟为离线会话）
	...
	public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession) {
		...
		for (Session session : getActiveSessions()){
			...
			// 包括离线并符合最后访问时间大于等于30分钟条件。
			if (includeLeave && DateUtils.pastMinutes(session.getLastAccessTime()) >= Long.parseLong(Global.getConfig("user.includeLeaveTime"))){
