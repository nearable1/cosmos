 ...
 */
package com.inbody.crm.modules.act.service;
...
import com.inbody.crm.modules.act.service.cmd.CreateAndTakeTransitionCmd;
import com.inbody.crm.modules.act.service.cmd.JumpTaskCmd;
import com.inbody.crm.modules.act.service.creator.ChainedActivitiesCreator;
import com.inbody.crm.modules.act.service.creator.MultiInstanceActivityCreator;
import com.inbody.crm.modules.act.service.creator.RuntimeActivityDefinitionEntityIntepreter;
import com.inbody.crm.modules.act.service.creator.SimpleRuntimeActivityDefinitionEntity;
import com.inbody.crm.modules.act.utils.ActUtils;
import com.inbody.crm.modules.act.utils.ProcessDefCache;
import com.inbody.crm.modules.act.utils.ProcessDefUtils;
...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		...
		if (act.getEndDate() != null){
			todoTaskQuery.taskCreatedBefore(addDay(act.getEndDate()));
		}
		...
		for (Task task : todoList) {
			...
			String applyUserCd = task.getProcessVariables().get("apply").toString();
			e.setCreateUser(getUserNameByCode(applyUserCd));
			e.setAssigneeName(getUserNameByCode(task.getAssignee()));
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
		...
		if (act.getEndDate() != null){
			toClaimQuery.taskCreatedBefore(addDay(act.getEndDate()));
		}
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	...
	public Page<Act> historicList(Page<Act> page, Act act){
		...
		if (act.getEndDate() != null){
			histTaskQuery.taskCompletedBefore(addDay(act.getEndDate()));
		}
		...
		for (HistoricTaskInstance histTask : histList) {
			HistoricActivityInstance na = getLastTask(histTask.getProcessInstanceId());
			Act e = new Act();
			e.setHistTask(histTask);
			e.setHistIns(na);
			e.setVars(histTask.getProcessVariables());
			String applyUserCd = histTask.getProcessVariables().get("apply").toString();
			e.setCreateUser(getUserNameByCode(applyUserCd));
			//e.setAssigneeName(getUserNameByCode(na.getAssignee()));
			e.setAssigneeName(getProcessingUserName(na));
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	...
	/**
	 * 获取当前处理人，如果当前处理人为用户组，则获取所有待签收的用户
	 * @param ins
	 * @return
	 */
	private String getProcessingUserName(HistoricActivityInstance ins)
	{
		if(ins.getActivityType().equals("endEvent"))
			return "";
		String assignee = ins.getAssignee();
		if(!StringUtils.isBlank(assignee))
			return getUserNameByCode(assignee);
		else{
			//获取最后一条任务
			List<HistoricTaskInstance> hisTaskList = historyService.createHistoricTaskInstanceQuery().processInstanceId(ins.getProcessInstanceId())
					.orderByTaskCreateTime().desc().orderByHistoricTaskInstanceEndTime().desc().list();
			if(hisTaskList != null && hisTaskList.size() > 0){
				...
				if(list != null && list.size() > 0){
					String candicateUserName = "";
					for(IdentityLink identityLink : list)
					{
						if(identityLink.getType().equals("assignee"))	continue;
						if(StringUtils.isBlank(identityLink.getGroupId()))	continue;
						//取得用户组对应的所有用户
						List<User> userList = userDao.getUsersByRoleEnname(identityLink.getGroupId());
//						List<org.activiti.engine.identity.User> userlist = 
//								ProcessEngines.getDefaultProcessEngine().getIdentityService()
//								.createUserQuery().memberOfGroup(identityLink.getGroupId()).list(); 
//						for(org.activiti.engine.identity.User user : userlist)
						for(User user : userList)
						{
							if(user != null && !StringUtils.isBlank(user.getName()))
							{
								candicateUserName += user.getName() + ",";
							}
						}
					}
					if(!StringUtils.isBlank(candicateUserName)){
						candicateUserName = candicateUserName.substring(0,candicateUserName.length()-1);
						candicateUserName = "待签收：["+candicateUserName+"]";
						return candicateUserName;
					}
					else{
						return "";
					}
	 ...
	 * @return
	 */
	private java.util.Date addDay(java.util.Date date)
	{
	 ...
	 * @return
	 */
	private HistoricActivityInstance getLastTask(String procInsId)
	{
...
//			return null;
		...
		for(HistoricActivityInstance ins : list){
			if(!StringUtils.isBlank(ins.getActivityName()))
				return ins;
		}
		return null;
	}
	 ...
	 */
	public List<Act> historicFlowList(String procInsId){
		List<HistoricTaskInstance> hisTaskList = historyService.createHistoricTaskInstanceQuery().processInstanceId(procInsId)
				.orderByTaskCreateTime().asc().orderByHistoricTaskInstanceEndTime().asc().list();
		
		List<Act> actList = Lists.newArrayList();
		for (int i=0; i<hisTaskList.size(); i++){
			HistoricTaskInstance histTask = hisTaskList.get(i);

			Act e = new Act();
			e.setHistTask(histTask);
			
			// 获取任务执行人名称
			if (StringUtils.isNotEmpty(histTask.getAssignee())){
			...
			// 撤回事件加入任务备注中
			if(!StringUtils.isBlank(histTask.getDeleteReason()) && histTask.getDeleteReason().equals("Revoke")){
				e.setComment("[申请人主动撤回]");
			}
			actList.add(e);
		}
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	 ...
	 */
	@Transactional(readOnly = false)
	public HistoricProcessInstance getProcHistoryIns(String procInsId) {
	 ...
	 */
	@Transactional(readOnly = false)
	public String startIminvoiceProcess(String procDefKey, String businessTable, String procId, String businessId, String title) {
		Map<String, Object> vars = Maps.newHashMap();
		return startIminvoiceProcess(procDefKey, businessTable, procId, businessId, title, vars);
	}
	 ...
	 */
	@Transactional(readOnly = false)
	public String startIminvoiceProcess(String procDefKey, String businessTable, String procId, String businessId, String title, Map<String, Object> vars) {
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId())
		...
		// 启动流程
		ProcessInstance procIns = runtimeService.startProcessInstanceByKey(procDefKey, businessTable+":"+procId, vars);
...
//		return ProcessDiagramGenerator.generateDiagram(bpmnModel, "png", activeActivityIds);
	...
	/**
	 * 根据用户登录名获取姓名，没有找到用户则返回登录名
	 * @param userCd
	 * @return
	 */
	private String getUserNameByCode(String userCd)
	{
		User user = UserUtils.getByLoginName(userCd);
		if(user != null)
			return user.getName();
		else
			return userCd;
