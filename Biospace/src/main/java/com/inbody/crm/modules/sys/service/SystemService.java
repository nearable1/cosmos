...
import com.inbody.crm.common.security.Digests;
import com.inbody.crm.common.security.shiro.session.SessionDAO;
...
import com.inbody.crm.common.web.Servlets;
...
import com.inbody.crm.modules.sys.security.SystemAuthorizingRealm;
import com.inbody.crm.modules.sys.utils.LogUtils;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	public User getUser(String id, String locale) {
		User user = UserUtils.get(id, locale);
		if(user == null && !locale.equals(Global.DEFAULT_LOCALE)){
			user = UserUtils.get(id, Global.DEFAULT_LOCALE);
			user.setLocaleId(locale);
		}
		return user;
	}
	...
	public void saveUser(User user) {
		User check = userDao.get(user.getId(), user.getLocaleId());
		if(check == null)
			user.setIsNewRecord(true);
		if (user.getIsNewRecord() || user.getIsNewTerm()){
			if(StringUtils.isNotBlank(user.getId())){
				// 清除原用户机构用户缓存
				User oldUser = userDao.getDefault(user.getId());
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public Menu getMenu(String id) {
		return getMenu(id, UserUtils.getUser().getCurrentLocaleId());
	}
	
	public Menu getMenu(String id, String locale) {
		Menu menu = menuDao.get(id, locale);
		if(menu == null && !locale.equals(Global.DEFAULT_LOCALE)){
			menu = menuDao.get(id, Global.DEFAULT_LOCALE);
			menu.setLocaleId(locale);
		}
		return menu;
	}
	...
	public void saveMenu(Menu menu) {
		...
		// 保存或更新实体
		if (StringUtils.isBlank(menu.getId()) || StringUtils.isBlank(menu.getTermId())){
			menu.preInsert();
			menuDao.insert(menu);
		}else{
			Menu check = menuDao.get(menu.getId(), menu.getLocaleId());
			if(check == null){
