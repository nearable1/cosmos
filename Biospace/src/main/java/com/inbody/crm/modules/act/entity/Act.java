 ...
 */
package com.inbody.crm.modules.act.entity;
...
import com.inbody.crm.common.utils.TimeUtils;
import com.inbody.crm.modules.act.utils.Variable;
...
public class Act extends BaseEntity<Act> {
	...
	private String mainBusinessId;  // 主业务绑定ID
...
//	private String procExecUrl; 	// 流程执行（办理）RUL
	...
	private HistoricProcessInstance procHistoricIns;	// 历史流程实例对象
...
//	private Variable taskVars; 	// 流程任务变量
	...
	public void setProcIns(ProcessInstance procIns) {
		...
		if (procIns != null && procIns.getBusinessKey() != null){
			...
			setBusinessId(ss[1]);
            if (ss.length > 2) {
                setMainBusinessId(ss[2]);
            }
		}
	}

	@JsonIgnore
	public HistoricProcessInstance getProcHistoricIns() {
		return procHistoricIns;
	}

	public void setProcHistoricIns(HistoricProcessInstance procHistoricIns) {
		this.procHistoricIns = procHistoricIns;
		if (procHistoricIns != null && procHistoricIns.getBusinessKey() != null){
			String[] ss = procHistoricIns.getBusinessKey().split(":");
			setBusinessTable(ss[0]);
			setBusinessId(ss[1]);
			if (ss.length > 2) {
			    setMainBusinessId(ss[2]);
...
//		}
		if (histTask!=null && histTask.getDurationInMillis() != null && histTask.getDurationInMillis() > 0){
			return TimeUtils.toTimeString(histTask.getDurationInMillis());
		}
		return "";
	}
