...
<body>
	...
	<form:form id="searchForm" modelAttribute="act" action="${ctx}/act/task/historic/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="act">
				...
				<tr>
					<td>${fns:abbr(not empty vars.map.title ? vars.map.title : task.id, 60)}
						<%-- 
					...
					<td>${act.histIns.activityName}
						<%-- 
						<a target="_blank" href="${pageContext.request.contextPath}/act/diagram-viewer?processDefinitionId=${task.processDefinitionId}&processInstanceId=${task.processInstanceId}">${act.histIns.activityName}</a>
						--%>
						<%--
					...
					<td>
						...
						<a target="_blank" href="${ctx}/act/task/histoicFlowShow?procInsId=${task.processInstanceId}">操作履历</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
