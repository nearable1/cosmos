	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="oaNotify">
			<tr>
				...
				<c:if test="${oaNotify.type == '5'}">
					<td><shiro:hasPermission name="rm:consulting:view"><a href="${ctx}/oa/oaNotify/${requestScope.oaNotify.self?'view':'form'}?id=${oaNotify.id}">
						咨询编号：${oaNotify.content}
						</a></shiro:hasPermission>
						<shiro:lacksPermission name="rm:consulting:view">咨询编号：${oaNotify.content}</shiro:lacksPermission>
					</td>
					<td>
						${fns:getDictLabel(oaNotify.type, 'oa_notify_type', '')}
					</td>
